package day45_ClassMore;

public class Food {

    /*
    Food - data: name, quantity, unit price, total price
     */
    String name;
    int quantity;
    double unitPrice;
    double totalPrice;


    /*
    - constructor
            - create a constructor that creates a Food object with the name
            - create a constructor that creates a Food object with the name and quantity
            - create a constructor that creates a Food object with the name, quantity, and unit
     */

    public Food (String name) {
        this.name = name;
    }
    public Food (String name, int quantity){
        this(name);  // we can put this("food");
        this.quantity=quantity;
    }
    public Food (String name, int quantity, double unitPrice){
        this(name, quantity);
        this.unitPrice=unitPrice;
        calculatePrice();

    }
    /*
     -> call calculatePrice() method here
     - method:  calculatePrice(): multiple the quantity by unit price and assign the value to the total price
     */
    public void calculatePrice(){
        totalPrice= quantity*unitPrice;
    }
    /*
       - toString() - print all the Food information
     */

       public String toString() {
        return name + " ~ "+ quantity + " X $" + unitPrice + " for $"+ totalPrice;

    }
}
